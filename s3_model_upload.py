...
``S3_MODEL_KEY`` (optional)
    Object key inside the bucket. Default: ``heart-disease/production/model.pkl``
...
def upload_production_model_pkl_if_configured(
    local_pkl: Path,
) -> str | None:
    """
    If ``S3_MODEL_BUCKET`` is set, upload ``local_pkl`` and return ``s3://bucket/key``.
